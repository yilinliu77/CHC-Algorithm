...
Model* model;
BVHAccel* leftBvhAccel, *rightBvhAccel;
...
void renderWithCHC() {
	...
	while (true) {
		...
		bool wasVisible = curNode->visible && (curNode->lastVisited 
											   == (gIsLeft? leftFrameID:rightFrameID) - 1);
		bool opened = wasVisible && !curNode->isLeaf;
		curNode->visible = false;
		curNode->lastVisited = (gIsLeft ? leftFrameID : rightFrameID);
		//===================version1========================
		if (!opened) {
	...
	++(gIsLeft ? leftFrameID : rightFrameID); //count the frames
	glBindVertexArray(0);
	glUseProgram(0);
...
void initTestData() {
	...
	//build tree
	leftBvhAccel = new BVHAccel(model->meshes, 500, SAH);
	rightBvhAccel = new BVHAccel(*leftBvhAccel);

	std::cout << "build successful" << std::endl;
	out << "=================Properties================\n";
	out << "Total Meshes:" << model->meshes.size() << "\n";
	out << "Total Triangles:" << model->totalTriangles << "\n";
	out << "Total Nodes for BVHAccel:" << leftBvhAccel->totalLinearNodes << "\n";
	out << "===========================================\n";
}
